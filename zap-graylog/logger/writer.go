...
func (w *GELFWriter) writeChunked(count int, b []byte) (n int, err error) {
	...
	for i := 0; i < count; i++ {
		...
		buffer.Reset()
		if _, err = buffer.Write(chunkedMagicBytes); err != nil {
			return 0, err
		}
		if _, err = buffer.Write(messageID); err != nil {
			return 0, err
		}
		if err = buffer.WriteByte(uint8(i)); err != nil {
			return 0, err
		}
		if err = buffer.WriteByte(uint8(count)); err != nil {
			return 0, err
		}
		if _, err = buffer.Write(b[off : off+chunkLen]); err != nil {
			return 0, err
		}
